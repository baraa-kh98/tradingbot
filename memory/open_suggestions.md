...
> آخر تحديث: 2026-05-27 (روتين صباحي — الأربعاء)

---

## 🟡 متوسطة الأولوية

### [6] XAUUSD — `_regime_check()` Exception بدون Logging *(2026-05-25، ثلاثة أيام)*
- **الملف:** `strategy/xauusd_signal.py:159-162`
- **المشكلة:** التعليق يقول "Log warning" لكن لا يوجد `_log.warning()`:
  ```python
  except Exception as e:
      # Log warning but don't block trading   ← كاذب: لا يوجد logging
      return True
  ```
- **الإصلاح المقترح:**
  ```python
      ...
      return True
  ```
  *(+ إضافة `_log = get_logger("xauusd_signal")` في بداية الملف)*
- **تاريخ الاكتشاف:** 2026-05-25
- **الحالة:** ⏳ ينتظر موافقة المستخدم (**يومان**)

### [4] XAUUSD — `_in_trade` check قبل `_regime_check()` *(2026-05-20، 8 أيام)*
...
- **الحالة:** ⏳ ينتظر موافقة المستخدم (**7 أيام** — يُفضّل تطبيقه مع Cache #4b)

### [4b] XAUUSD Regime Check — Cache مفقود *(منذ 2026-05-16، 11 يوم)* ⚡ عاجل — قبل Core PCE غداً
...
- **الحالة:** ⏳ ينتظر موافقة المستخدم (**11 يوم** ← يُوصى بالتطبيق مع #4 — قبل Core PCE غداً)
...
### [3] إضافة "strategy" key لكل Signal Generator *(منذ 2026-05-12، 15 يوم)*
...
- **الحالة:** ⏳ ينتظر موافقة المستخدم (**15 يوم** ← الأقدم في القائمة)
...
### [15] Core PCE الخميس 28 مايو وتأثيره على البوت *(2026-05-26 — غداً!)*
- **الموضوع:** Core PCE Deflator — أهم بيانات التضخم الأمريكي (الفضّل لـ Fed)
- **الوقت:** 12:30 UTC الخميس 28 مايو 2026
- **الأزواج المتأثرة:** كل الأزواج — PCE يحرك الدولار بعنف لـ 30-60 دقيقة
- **توصية:** مراقبة جودة الإشارات في نافذة 13:00-15:00 UTC بعد PCE مباشرة
- **متى نقرر؟:** بعد مراقبة رد فعل السوق غداً الخميس 28/5
- **الحالة:** 🔍 تحت المراقبة — **مراقبة مكثّفة غداً**
