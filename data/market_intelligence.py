...
class CurrencyStrength:
    """حساب قوة كل عملة من بيانات الأزواج"""
        ...
        try:
            ...
            for symbol, (base, quote) in self.PAIRS.items():
                try:
                    ...
                    # العملة الأساسية تقوى لو الزوج طلع
            ...
            if strength:
                sorted_str = dict(sorted(strength.items(), key=lambda x: x[1], reverse=True))
                return sorted_str
